...
setwd("C:/Users/franc/OneDrive - Universidad Católica de Chile/Diversidad animal/Escritorio/Revisión SciRep")


## cargar red formato xlsx
library(readxl)
bunny_fw <- read_excel("Lista interacción post revisión SciRep.xlsx", sheet = "Links")
bunny_fw
head(bunny_fw)
...
nodes_fw <- network::network.vertex.names(net_bunny) #indica el nombre de los nodos (el conejo esel 1)

##Buscando la posición del nodo que se llama 1 para ser removido de la red
...
##Compruebas que realmente se eliminó el nodo que se llama "1"
...
#Ok removemos la fila 1 que corresponde al grado del conejo antes de la extinción 
grado_s1 <- grado[-30,]  

#ahora si, podemos unir ambos objetos 
delta <- cbind(grado_s1,grado1) 
